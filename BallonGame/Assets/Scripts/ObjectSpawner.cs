...
{     
    ...
    public void Update()
    {
        float distancetoHorizon_1 = Vector3.Distance(Player.gameObject.transform.position, SpawnObs_Position);

        float distancetoHorizon_2 = Vector3.Distance(Player.gameObject.transform.position, SpawnCoin_Pos);
        /*Variavel que guarda a distancia entre um ponta A e B, neste caso distancia do player em relação ao SpawnObs_Position*/

        float distancetoHorizon_3 = Vector3.Distance(Player.gameObject.transform.position, SpawnJump_Pos);
